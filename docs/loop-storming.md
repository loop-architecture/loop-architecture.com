...
> The [example architecture](../examples/entropy-data.looparch.yaml) on this site came straight out of
...
Keep it to 5–9 people per wall. Split into multiple walls if larger.
...
### 1. Define the systems (15–20 min)
...
### 2. Brainstorm the loops (15–20 min)
...
### 3. Draw the loops as edges, and name them (20–30 min)
...
```bash
looparch init entropy-data                    # scaffold the architecture
looparch validate entropy-data.looparch.yaml
looparch diagram entropy-data.looparch.yaml   # all your loops in one diagram
looparch publish entropy-data.looparch.yaml   # turn each loop into a Claude Code routine
```
